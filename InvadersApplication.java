...
public class InvadersApplication extends JFrame implements Runnable, KeyListener {
	...
	public InvadersApplication() {
		// Display the window, center on the screen
		...
		//crate and initialize the player's spaceship
		...
		//send keyboard event arriving into this JFrame back to its own event handlers
		addKeyListener(this);
		...
		while ( 1==1 ) {
			...
			if (isGameInProgress) {
				...
				if (alienDirectionReversalNeeded) {
					for (int i=0; i<NUMALIENS; i++) {
						if (AliensArray[i].isAlive) {
							AliensArray[i].reverseDrirection();
				...
				while (iterator.hasNext()) {
					...
					if (b.move()) {
						//true was returned by move method if bullet needs destroying due to going offscreen iterator.remove is a safe way to remove from the ArrayList while iterating thru it
						iterator.remove();
					}
					else {
						...
						for (int i=0; i<NUMALIENS; i++) {
							if (AliensArray[i].isAlive) {
								...
								if (isCollision(x1,x2,y1,y2,w1,w2,h1,h2)) {
									...
									iterator.remove(); //this is a safe way to remove from an ArrayList while itrating 
									score+=10;
									if (score>highscore)
										highscore = score;
	...
	//helper method for drawing strings centered at specified position
